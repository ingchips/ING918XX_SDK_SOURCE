...
void platform_get_heap_status(platform_heap_status_t *status)
{
    ...
    pool_size = LOS_MemPoolSizeGet(m_aucSysMem0);
    used_size = LOS_MemTotalUsedGet(m_aucSysMem0);
    status->bytes_free = pool_size - used_size;
    status->bytes_minimum_ever_free = used_size;
}
