...
void AES_128(const uint8_t *k, const uint8_t *p, uint8_t *c){
    uint8_t r_k[16], r_p[16];
    reverse_128(k, r_k);
    reverse_128(p, r_p);
    if( ll_aes_encrypt(r_k, r_p, c) ){
        printf("aes busy!!!\n");
    }
}

void print128(unsigned char *bytes) { int j; for (j=0; j<16;j++) { printf("%02x",bytes[j]); if ( (j%4) == 3 ) printf(" "); } }

void print96(unsigned char *bytes) { int j; for (j=0; j<12;j++) { printf("%02x",bytes[j]); if ( (j%4) == 3 ) printf(" "); } }
