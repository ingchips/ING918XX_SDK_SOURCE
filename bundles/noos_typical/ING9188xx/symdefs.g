att_dispatch_client_can_send_now = 0x00005a39;
att_dispatch_client_request_can_send_now_event = 0x00005a3f;
att_dispatch_register_client = 0x00005a45;
att_dispatch_register_server = 0x00005a59;
att_dispatch_server_can_send_now = 0x00005a6d;
att_dispatch_server_request_can_send_now_event = 0x00005a73;
att_emit_general_event = 0x00005b25;
att_server_can_send_packet_now = 0x00006239;
att_server_deferred_read_response = 0x0000623d;
att_server_get_mtu = 0x00006255;
att_server_indicate = 0x000062cd;
att_server_init = 0x00006351;
att_server_notify = 0x0000638d;
att_server_register_packet_handler = 0x000064a5;
att_server_request_can_send_now_event = 0x000064b1;
att_set_db = 0x000064cd;
att_set_read_callback = 0x000064e1;
att_set_write_callback = 0x000064ed;
bd_addr_cmp = 0x0000665d;
bd_addr_copy = 0x00006663;
bd_addr_to_str = 0x0000666d;
big_endian_read_16 = 0x000066a5;
big_endian_read_32 = 0x000066ad;
big_endian_store_16 = 0x000066c1;
big_endian_store_32 = 0x000066cd;
btstack_config = 0x00006805;
btstack_memory_pool_create = 0x00006953;
btstack_memory_pool_free = 0x0000697d;
btstack_memory_pool_get = 0x000069dd;
btstack_push_user_msg = 0x000069f9;
char_for_nibble = 0x00006c89;
gap_add_dev_to_periodic_list = 0x00007681;
gap_add_whitelist = 0x00007699;
gap_aes_encrypt = 0x000076ad;
gap_clear_white_lists = 0x000076f1;
gap_clr_adv_set = 0x00007701;
gap_clr_periodic_adv_list = 0x00007711;
gap_create_connection_cancel = 0x0000773d;
gap_default_periodic_adv_sync_transfer_param = 0x0000774d;
gap_disconnect = 0x00007765;
gap_disconnect_all = 0x00007791;
gap_ext_create_connection = 0x00007839;
gap_get_connection_parameter_range = 0x00007929;
gap_le_read_channel_map = 0x00007965;
gap_periodic_adv_create_sync = 0x000079d9;
gap_periodic_adv_create_sync_cancel = 0x000079fd;
gap_periodic_adv_set_info_transfer = 0x00007a0d;
gap_periodic_adv_sync_transfer = 0x00007a25;
gap_periodic_adv_sync_transfer_param = 0x00007a3d;
gap_periodic_adv_term_sync = 0x00007a59;
gap_read_antenna_info = 0x00007aed;
gap_read_periodic_adv_list_size = 0x00007afd;
gap_read_phy = 0x00007b0d;
gap_read_remote_info = 0x00007b21;
gap_read_remote_used_features = 0x00007b35;
gap_read_rssi = 0x00007b49;
gap_remove_whitelist = 0x00007b5d;
gap_rmv_adv_set = 0x00007be9;
gap_rmv_dev_from_periodic_list = 0x00007bfd;
gap_rx_test_v2 = 0x00007c15;
gap_rx_test_v3 = 0x00007c2d;
gap_set_adv_set_random_addr = 0x00007c7d;
gap_set_connection_cte_request_enable = 0x00007cc9;
gap_set_connection_cte_response_enable = 0x00007ce5;
gap_set_connection_cte_rx_param = 0x00007cf9;
gap_set_connection_cte_tx_param = 0x00007d55;
gap_set_connection_parameter_range = 0x00007da9;
gap_set_connectionless_cte_tx_enable = 0x00007dc1;
gap_set_connectionless_cte_tx_param = 0x00007dd5;
gap_set_connectionless_iq_sampling_enable = 0x00007e35;
gap_set_data_length = 0x00007e99;
gap_set_def_phy = 0x00007eb5;
gap_set_ext_adv_data = 0x00007ecd;
gap_set_ext_adv_enable = 0x00007ee5;
gap_set_ext_adv_para = 0x00007f61;
gap_set_ext_scan_enable = 0x00008041;
gap_set_ext_scan_para = 0x00008059;
gap_set_ext_scan_response_data = 0x00008101;
gap_set_host_channel_classification = 0x00008119;
gap_set_periodic_adv_data = 0x0000812d;
gap_set_periodic_adv_enable = 0x000081a1;
gap_set_periodic_adv_para = 0x000081b5;
gap_set_periodic_adv_rx_enable = 0x000081cd;
gap_set_phy = 0x000081e1;
gap_set_random_device_address = 0x000081fd;
gap_start_ccm = 0x00008261;
gap_test_end = 0x00008295;
gap_tx_test_v2 = 0x000082a5;
gap_tx_test_v4 = 0x000082bd;
gap_update_connection_parameters = 0x000082e5;
gap_vendor_tx_continuous_wave = 0x00008309;
gatt_client_cancel_write = 0x00008831;
gatt_client_discover_characteristic_descriptors = 0x00008857;
gatt_client_discover_characteristics_for_handle_range_by_uuid128 = 0x00008897;
gatt_client_discover_characteristics_for_handle_range_by_uuid16 = 0x000088e7;
gatt_client_discover_characteristics_for_service = 0x00008937;
gatt_client_discover_primary_services = 0x0000896d;
gatt_client_discover_primary_services_by_uuid128 = 0x0000899f;
gatt_client_discover_primary_services_by_uuid16 = 0x000089e3;
gatt_client_execute_write = 0x00008a1f;
gatt_client_find_included_services_for_service = 0x00008a45;
gatt_client_get_mtu = 0x00008a73;
gatt_client_is_ready = 0x00008b15;
gatt_client_listen_for_characteristic_value_updates = 0x00008b2b;
gatt_client_prepare_write = 0x00008b4d;
gatt_client_read_characteristic_descriptor_using_descriptor_handle = 0x00008b89;
gatt_client_read_long_characteristic_descriptor_using_descriptor_handle = 0x00008bb3;
gatt_client_read_long_characteristic_descriptor_using_descriptor_handle_with_offset = 0x00008bb9;
gatt_client_read_long_value_of_characteristic_using_value_handle = 0x00008be7;
gatt_client_read_long_value_of_characteristic_using_value_handle_with_offset = 0x00008bed;
gatt_client_read_multiple_characteristic_values = 0x00008c1b;
gatt_client_read_value_of_characteristic_using_value_handle = 0x00008c4b;
gatt_client_read_value_of_characteristics_by_uuid128 = 0x00008c79;
gatt_client_read_value_of_characteristics_by_uuid16 = 0x00008cc5;
gatt_client_register_handler = 0x00008d11;
gatt_client_reliable_write_long_value_of_characteristic = 0x00008d1d;
gatt_client_signed_write_without_response = 0x0000914d;
gatt_client_write_characteristic_descriptor_using_descriptor_handle = 0x00009211;
gatt_client_write_client_characteristic_configuration = 0x0000924b;
gatt_client_write_long_characteristic_descriptor_using_descriptor_handle = 0x0000929d;
gatt_client_write_long_characteristic_descriptor_using_descriptor_handle_with_offset = 0x000092ad;
gatt_client_write_long_value_of_characteristic = 0x000092e9;
gatt_client_write_long_value_of_characteristic_with_offset = 0x000092f9;
gatt_client_write_value_of_characteristic = 0x00009335;
gatt_client_write_value_of_characteristic_without_response = 0x0000936b;
hci_add_event_handler = 0x0000a8ad;
hci_power_control = 0x0000b0b1;
hci_register_acl_packet_handler = 0x0000b265;
kv_commit = 0x0000b871;
kv_get = 0x0000b8c9;
kv_init = 0x0000b8e1;
kv_put = 0x0000b949;
kv_remove = 0x0000b9c1;
kv_remove_all = 0x0000b9fd;
kv_value_modified = 0x0000ba41;
kv_visit = 0x0000ba45;
l2cap_can_send_fixed_channel_packet_now = 0x0000bb05;
l2cap_can_send_packet_now = 0x0000bb09;
l2cap_create_channel = 0x0000bcc1;
l2cap_disconnect = 0x0000bdf9;
l2cap_get_remote_mtu_for_local_cid = 0x0000c06d;
l2cap_init = 0x0000c4ed;
l2cap_le_send_flow_control_credit = 0x0000c535;
l2cap_max_le_mtu = 0x0000c7b9;
l2cap_max_mtu = 0x0000c7bd;
l2cap_next_local_cid = 0x0000c7c1;
l2cap_next_sig_id = 0x0000c7d1;
l2cap_register_fixed_channel = 0x0000c869;
l2cap_register_packet_handler = 0x0000c885;
l2cap_register_service = 0x0000c891;
l2cap_request_can_send_fix_channel_now_event = 0x0000c979;
l2cap_request_can_send_now_event = 0x0000c99d;
l2cap_request_connection_parameter_update = 0x0000c9b7;
l2cap_require_security_level_2_for_outgoing_sdp = 0x0000c9e9;
l2cap_send = 0x0000cd7d;
l2cap_send_connectionless = 0x0000cdf5;
l2cap_send_connectionless3 = 0x0000ce85;
l2cap_send_echo_request = 0x0000cf1d;
l2cap_send_signaling_le = 0x0000cf81;
l2cap_unregister_service = 0x0000cfd9;
le_device_db_add = 0x0000d03d;
le_device_db_find = 0x0000d111;
le_device_db_from_key = 0x0000d13d;
le_device_db_iter_cur = 0x0000d145;
le_device_db_iter_cur_key = 0x0000d149;
le_device_db_iter_init = 0x0000d14d;
le_device_db_iter_next = 0x0000d155;
le_device_db_remove_key = 0x0000d17b;
ll_free = 0x0000d1a7;
ll_hint_on_ce_len = 0x0000d1b1;
ll_legacy_adv_set_interval = 0x0000d1e9;
ll_malloc = 0x0000d1f9;
ll_query_timing_info = 0x0000d331;
ll_scan_set_fixed_channel = 0x0000d3d5;
ll_set_adv_access_address = 0x0000d4e9;
ll_set_adv_coded_scheme = 0x0000d4f5;
ll_set_conn_coded_scheme = 0x0000d525;
ll_set_conn_latency = 0x0000d551;
ll_set_conn_tx_power = 0x0000d581;
ll_set_def_antenna = 0x0000d5c9;
ll_set_initiating_coded_scheme = 0x0000d5e5;
ll_set_max_conn_number = 0x0000d5f1;
nibble_for_char = 0x0001e271;
platform_32k_rc_auto_tune = 0x0001e30d;
platform_32k_rc_tune = 0x0001e389;
platform_calibrate_32k = 0x0001e39d;
platform_config = 0x0001e3a1;
platform_get_task_handle = 0x0001e461;
platform_get_us_time = 0x0001e479;
platform_get_version = 0x0001e47d;
platform_hrng = 0x0001e485;
platform_os_idle_resumed_hook = 0x0001e48b;
platform_patch_rf_init_data = 0x0001e491;
platform_post_sleep_processing = 0x0001e49d;
platform_pre_sleep_processing = 0x0001e4a3;
platform_pre_suppress_ticks_and_sleep_processing = 0x0001e4a9;
platform_printf = 0x0001e4ad;
platform_raise_assertion = 0x0001e4c1;
platform_rand = 0x0001e4d5;
platform_read_info = 0x0001e4d9;
platform_read_persistent_reg = 0x0001e4f5;
platform_reset = 0x0001e505;
platform_set_evt_callback = 0x0001e529;
platform_set_irq_callback = 0x0001e53d;
platform_set_rf_clk_source = 0x0001e575;
platform_set_rf_init_data = 0x0001e581;
platform_set_rf_power_mapping = 0x0001e58d;
platform_shutdown = 0x0001e599;
platform_switch_app = 0x0001e59d;
platform_trace_raw = 0x0001e5c9;
platform_write_persistent_reg = 0x0001e5e1;
printf_hexdump = 0x0001e5f1;
reverse_128 = 0x0001e971;
reverse_24 = 0x0001e977;
reverse_48 = 0x0001e97d;
reverse_56 = 0x0001e983;
reverse_64 = 0x0001e989;
reverse_bd_addr = 0x0001e98f;
reverse_bytes = 0x0001e995;
sm_add_event_handler = 0x0001ec55;
sm_address_resolution_lookup = 0x0001edad;
sm_authenticated = 0x0001f0f9;
sm_authorization_decline = 0x0001f107;
sm_authorization_grant = 0x0001f127;
sm_authorization_state = 0x0001f147;
sm_bonding_decline = 0x0001f161;
sm_config = 0x0001f581;
sm_config_conn = 0x0001f599;
sm_encryption_key_size = 0x0001f71f;
sm_just_works_confirm = 0x0001fc4d;
sm_le_device_key = 0x0001ff89;
sm_passkey_input = 0x0002001f;
sm_private_random_address_generation_get = 0x000203c5;
sm_private_random_address_generation_get_mode = 0x000203cd;
sm_private_random_address_generation_set_mode = 0x000203d9;
sm_private_random_address_generation_set_update_period = 0x00020401;
sm_register_oob_data_callback = 0x0002053d;
sm_request_pairing = 0x00020549;
sm_send_security_request = 0x00020f43;
sm_set_accepted_stk_generation_methods = 0x00020f69;
sm_set_authentication_requirements = 0x00020f75;
sm_set_encryption_key_size_range = 0x00020f81;
sscanf_bd_addr = 0x000212d1;
sysSetPublicDeviceAddr = 0x00021639;
uuid128_to_str = 0x00021dc5;
uuid_add_bluetooth_prefix = 0x00021e1d;
uuid_has_bluetooth_prefix = 0x00021e3d;
