                  ...
                  uint8_t *pKey, uint16_t keybits, uint8_t *iv)
{
    ...
    if (UTILS_AES_ENCRYPT == mode) {
        ...
        if (ret != 0) {
            ble_qiot_log_e("Set encry key err,ret:%d", ret);
            ret = BLE_QIOT_RS_ERR;
            goto exit;
        }
        ...
        if (ret != 0) {
            ble_qiot_log_e("Set dencry key err,ret:%d", ret);
            ret = BLE_QIOT_RS_ERR;
            goto exit;
        }
    }
    ...
    if (ret != 0) {
        ble_qiot_log_e("encryt err,ret:%d", ret);
        ret = BLE_QIOT_RS_ERR;
        goto exit;
    } else {
        ret = BLE_QIOT_RS_OK;
...
int aes_sample(int verbose)
{
    ...
    if (BLE_QIOT_RS_OK == utils_aes_cbc((uint8_t *)dataIn, sLen, (uint8_t *)dataIn, sLen + UTILS_AES_BLOCK_LEN,
